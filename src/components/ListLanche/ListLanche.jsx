...
const ListLanche = () => {
  ...
  const updateLanche = async () => {
    try {
      ...
      setLanches(
        lanches.map((lanche) =>
          lanche.id === selectedLanche.id ? formData : lanche
        )
      );
            ...
            {lanches.map((lanche) => (
              <tr key={lanche.id}>
                ...
                <td>
                  {lanche.imagem && (
                    <img
                      src={`http://localhost:3000/${lanche.imagem}`}
                      alt={`Imagem de ${lanche.nome}`}
                      className="lanche-imagem"
                    />
                  )}
                </td>
                <td className="button-group">
                  <button
                    ...
                    onClick={() => handleEditClick(lanche)}
                  >
                    Editar
                  </button>
                  <button
      ...
      {selectedLanche && (
        <div className="edit-form">
          ...
          <div className="button-group">
            <button className="save-button" onClick={updateLanche}>
              Salvar
            </button>
            <button
              className="cancel-button"
              onClick={() => setSelectedLanche(null)}
            >
              Cancelar
            </button>
          </div>
